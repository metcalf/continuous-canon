...
int main(int argc, char **argv){
	...
	char macaddr[64], user[64], job[64], model_number[64];
...
static int _ij_network_print(
	...
	int working=0, count=0, tmp_ret=CNNL_RET_SUCCESS;
	
	// clear buffer
	setbuf(stdout, NULL);
	setbuf(stderr, NULL);
	memset(ipaddr, 0x00, STRING_SHORT);
	memset(hostname, 0x00, STRING_SHORT);
	...
	// get hostname
	if (gethostname(hostname, STRING_SHORT-1) != 0){
		errcode = 1;
...
find_printer:
		...
		*/
		...
		if (CNNL_OpenEx(hnd, ipaddr, CNNET_TYPE_PRINTER, 3, 6000) != CNNL_RET_SUCCESS){
			errcode = 1;
			fprintf(stderr, "ERROR: error occurred while preparing\n");
			goto error_open;
		}
		...
		if (CNNL_CheckVersion(hnd, 3, 9000) != CNNL_RET_SUCCESS){
			fprintf(stderr, "ERROR: error occurred while preparing\n");
			errcode = 1;
			goto error_close;
		}
		...
		// check command type
		if (CNNL_GetCommandType(hnd, &command, 3, 9000) != CNNL_RET_SUCCESS){
			fprintf(stderr, "ERROR: error occurred while preparing\n");
			errcode = 1;
			goto error_close;
		}
		...
		packet_size = get_packet_size(atoi(model_number));
		...
		*/
		...
		/* set timeout */
		CNNL_SetTimeout(hnd, SESSION_TIMEOUT, 3, 9000);
		...
		*/
		...
		for (count=0; working!=1 && count<10; sleep(1)){
			...
			if (tmp_ret == CNNL_RET_SUCCESS){
				break;
			} else if (tmp_ret == CNNL_RET_FAILURE){
				goto error_printing;
...
unsigned long fread_with_retry(char *rbuffer, unsigned long bufsize, FILE *fp){
	...
	while (!(mode & CNNL_JOB_CANCELLED)){
		...
		if(FD_ISSET(0,&fds)){
			...
			// keep connection
			if (CNNL_GetTimeout(hnd, &d_time, 3, 9000) != CNNL_RET_SUCCESS){
				mode |= CNNL_JOB_ERROR;
				return 0;
			} else {
				errorcount=0;
