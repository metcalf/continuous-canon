...
int sendData(CNNLHANDLE hnd, void *buffer, unsigned long size, int sendmode, FILE *fp){
	...
	while (!(mode & CNNL_JOB_ERROR) && !(mode & CNNL_JOB_CANCELLED) && wsize < size){
		...
		ret = CNNL_DataWrite(hnd, buffer+wsize, req_size, &res_size, 3, 3000);
...
int dispatchCommandIVEC(CNNLHANDLE hnd, int type){
	...
	while (!(mode & CNNL_JOB_ERROR) && !(mode & CNNL_JOB_CANCELLED)){
		...
		usleep(100*1000);
		if (CNNL_DataRead(hnd, read_buffer, &rsize, sizeof(read_buffer), 3, 5000) == CNNL_RET_SUCCESS){
			
			// check response
			if ((ret=checkPrintCommand(type, read_buffer, (short)rsize, job_id, sizeof(job_id))) == CNCL_OK){
...
static int getPrinterStatus(CNNLHANDLE hnd, char *buf, unsigned long size){
	...
	if (CNNL_DataRead(hnd, read_buffer, &rsize, sizeof(read_buffer), 3, 5000) == CNNL_RET_SUCCESS){
		...
		if (getStatusCommand(read_buffer, (short)rsize, status_buffer, sizeof(status_buffer), (short*)(&ssize)) == CNCL_OK){
