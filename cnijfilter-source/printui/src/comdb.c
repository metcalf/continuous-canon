...
gboolean IsIVECModel()
{
	...
	for( i = 0 ; i < g_uidb.dbsize ; i++, lpdb++ )
	{
		if( lpdb->nObjectID == CNCL_COMMAND_TYPE
		 && lpdb->nValue == CND_COMMAND_TYPE_01 )
				result = TRUE;
	}
...
short GetCurrentInkCartridgeSettings()
{
	...
	/* Ver.2.90: InkCartridgeSettings is checked and changed at cif program. */
#if 0
...
//	if( name != '\0' )
	if( *name != '\0' )
		name = LookupText(g_keytext_list, name);	//Ver.2.90
