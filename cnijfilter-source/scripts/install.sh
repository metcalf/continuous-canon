...
L_INST_COM_02_01="Usage: %s [ --uninstall | --version ]\n"
...
P_FUNC_show_copyright()
{
	...
	p_copyright2="Copyright CANON INC. 2001-2010"
...
if [ ${0##*/} = $C_install_script_fname ]; then
	...
	if test ! -f $C_fpath_common; then
		printf "$L_INST_COM_01_03"
		exit
	fi
	if test ! -f $C_fpath_depend; then
		printf "$L_INST_COM_01_03"
		exit
	fi
	...
	C_FUNC_rpm_install_process()
	{
		...
		if [ $? -eq 0 ]; then
			...
			c_installed_pkg_ver=`echo ${c_installed_pkg_ver%%.$C_arch}`
